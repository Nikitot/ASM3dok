...
void impositionOptFlow(Mat &frame, vector<Point> &faceKeyPoints, Mat &gray, Mat &prevgray){
	...
	if (prevgray.data)
	{
		...
		calcOpticalFlowFarneback(prevgray,//prev – first 8-bit single-channel input image.
								 gray,//next – second input image of the same size and the same type as prev.
								 flow,//flow – computed flow image that has the same size as prev and type CV_32FC2.
								 0.5,//pyr_scale – parameter, specifying the image scale (<1) to build pyramids for each image; pyr_scale=0.5 means a classical pyramid, where each next layer is twice smaller than the previous one.
								 3,//levels – number of pyramid layers including the initial image; levels=1 means that no extra layers are created and only the original images are used.
								 15,//winsize – averaging window size; larger values increase the algorithm robustness to image noise and give more chances for fast motion detection, but yield more blurred motion field.
								 3,//iterations – number of iterations the algorithm does at each pyramid level.
								 5,//poly_n – size of the pixel neighborhood used to find polynomial expansion in each pixel; larger values mean that the image will be approximated with smoother surfaces, yielding more robust algorithm and more blurred motion field, typically poly_n =5 or 7.
								 1.1,//		poly_sigma – standard deviation of the Gaussian that is used to smooth derivatives used as a basis for the polynomial expansion; for poly_n=5, you can set poly_sigma=1.1, for poly_n=7, a good value would be poly_sigma=1.5.
								 0);
								 /*
								 flags –
...
int main(int argc, char* argv[])
{
	...
	while (1){
		...
		framePointsÑoloring(frame, faceKeyPoints, faceFrameInfo.thisCenter,0);
